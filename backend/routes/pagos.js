...
// Corrige el monto o las notas de un pago ya registrado (solo gerente, ajuste financiero)
router.patch('/:id', requireGerente, async (req, res) => {
  ...
  try {
    ...
    const { rows: [pago] } = await client.query(
      'SELECT * FROM pagos WHERE id = $1 AND empresa_id = $2', [req.params.id, req.user.empresa_id]
    );
    if (!pago) {
    ...
    const { rows: [cuenta] } = await client.query(
      'SELECT * FROM cuentas_por_pagar WHERE id = $1 FOR UPDATE', [pago.cuenta_id]
    );
    const { rows: [{ t }] } = await client.query(
      'SELECT COALESCE(SUM(monto), 0) AS t FROM pagos WHERE cuenta_id = $1 AND id != $2',
      [pago.cuenta_id, pago.id]
    );
    const saldoDisponible = cuenta.monto_total - parseFloat(t);
    if (monto > saldoDisponible + 0.01) {
      await client.query('ROLLBACK');
      return res.status(400).json({ error: `El pago no puede ser mayor al saldo disponible (${saldoDisponible})` });
    }
    const { rows: [actualizado] } = await client.query(
      'UPDATE pagos SET monto = $1, notas = $2 WHERE id = $3 RETURNING *',
      [monto, notas, pago.id]
    );
    await actualizarPagadaEnCP(client, pago.cuenta_id);
